...
import {
  ...
  LineChart, Line, RadialBarChart, RadialBar, PieChart, Pie, Cell, AreaChart, Area
} from "recharts";
import { monthsData, monthKeys, projectColors, trendData } from "./data/crmData";
import { Bot, Send, X, ChevronDown, TrendingUp, TrendingDown, IndianRupee, Building2, FileCheck, AlertCircle, BarChart2, Layers, MessageSquare } from "lucide-react";
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-accent)", borderRadius: 8, padding: "10px 14px", fontSize: 12 }}>
...
function AIChatPanel({ open, onClose, currentData, monthKey }) {
  const [messages, setMessages] = useState([
    { role: "assistant", text: "👋 Hello! I'm your CRM AI Assistant powered by Gemini. Ask me anything about your collections, outstanding amounts, project performance, or registrations." }
  ]);
...
${d.project_tva.map(p => `${p.name}: Target=₹${p.target}Cr, Achievement=₹${p.achievement}Cr (${p.achievement_pct}%)`).join("\n")}

Category-wise targets:
${d.category_tva.map(c => `${c.category}: Target=₹${c.target}Cr, Achievement=₹${c.achievement}Cr`).join("\n")}

Be concise, insightful, and actionable. Use ₹ and Cr notation for amounts. Give recommendations when appropriate.`;
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: "assistant", text: `Error: ${err.message}. Please check your API key.` }]);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{
      position: "fixed", right: 24, bottom: 24, width: 380, height: 580,
      background: "var(--bg-card)", border: "1px solid var(--border-accent)",
      borderRadius: 16, display: "flex", flexDirection: "column",
      boxShadow: "0 24px 80px rgba(0,0,0,0.6)", zIndex: 1000,
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 14, fontFamily: "var(--font-display)" }}>CRM AI Assistant</div>
          <div style={{ fontSize: 11, color: "var(--accent-green)", display: "flex", alignItems: "center", gap: 4 }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--accent-green)", display: "inline-block" }} />
            Powered by Gemini
          </div>
        </div>
        <button onClick={onClose} style={{ marginLeft: "auto", background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", padding: 4 }}>
          <X size={16} />
        </button>
      </div>
      ...
      {showApiInput && (
        <div style={{ padding: 16, borderBottom: "1px solid var(--border)", background: "rgba(79,179,255,0.05)" }}>
          <div style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 8 }}>
            Enter your Gemini API key (free at ai.google.dev)
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              placeholder="AIza..."
              style={{ flex: 1, background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: 6, padding: "6px 10px", color: "var(--text-primary)", fontSize: 12, outline: "none" }}
            />
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
        {messages.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{
              maxWidth: "82%",
              background: m.role === "user" ? "linear-gradient(135deg, var(--accent-blue), #2563eb)" : "var(--bg-secondary)",
        ...
        {loading && (
          <div style={{ display: "flex", gap: 6, padding: "10px 13px", background: "var(--bg-secondary)", borderRadius: "12px 12px 12px 2px", width: "fit-content", border: "1px solid var(--border)" }}>
            {[0, 1, 2].map(i => (
              <span key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--accent-blue)", animation: `pulse-glow 1.2s ease-in-out ${i * 0.2}s infinite` }} />
            ))}
          </div>
        )}
        <div ref={bottomRef} />
      ...
      <div style={{ padding: "0 16px 8px", display: "flex", gap: 6, flexWrap: "wrap" }}>
        {["Top performer?", "Outstanding risk", "Reg. status", "Monthly trend"].map(s => (
          <button key={s} onClick={() => setInput(s)} style={{ fontSize: 11, padding: "4px 10px", background: "rgba(79,179,255,0.1)", border: "1px solid rgba(79,179,255,0.2)", borderRadius: 20, color: "var(--accent-blue)", cursor: "pointer" }}>{s}</button>
        ))}
      </div>
      ...
      <div style={{ padding: "0 16px 16px", display: "flex", gap: 8 }}>
        <input
          ...
          style={{ flex: 1, background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: 10, padding: "10px 14px", color: "var(--text-primary)", fontSize: 13, outline: "none", transition: "border-color 0.2s" }}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-primary)" }}>
          ...
          <nav style={{ display: "flex", gap: 2, marginLeft: 24 }}>
            {tabs.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                style={{
                  background: activeTab === t.id ? "rgba(79,179,255,0.12)" : "none",
            ...
            <div style={{ position: "relative" }}>
              <select
                ...
                style={{
                  ...
                  borderRadius: 8, padding: "8px 36px 8px 14px", color: "var(--text-primary)",
                  fontSize: 13, fontWeight: 600, cursor: "pointer", outline: "none",
                  appearance: "none", fontFamily: "var(--font-display)"
                }}>
      ...
      <main style={{ maxWidth: 1400, margin: "0 auto", padding: "24px" }}>
        ...
        {activeTab === "overview" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 8 }}>
              <KpiCard label="CRM Achievement" value={cr(data.crm_achievement)}
                sub={`Target: ${cr(data.crm_target)} · ${pct(data.crm_pct)}`}
                color={pctColor(data.crm_pct)} icon={IndianRupee}
                trend={collectionTrend} />
              <KpiCard label="Monthly Collection" value={cr(total.monthly_collection)}
                sub={`Daily: ${cr(total.daily_collection)}`}
                icon={TrendingUp} trend={collectionTrend} />
              <KpiCard label="Total Outstanding" value={cr(total.outstanding)}
                sub={`${total.pending_reg} pending registrations`}
                color="var(--accent-amber)" icon={AlertCircle} trend={outstandingTrend} />
              <KpiCard label="Live Bookings" value={total.live_bookings.toLocaleString()}
                sub={`${total.monthly_registrations} registered this month`}
                icon={Building2} />
              <KpiCard label="Demand Raised" value={cr(total.demand_raised)}
                sub={`Collected: ${cr(total.collection_till_date)}`}
                color="var(--accent-cyan)" icon={FileCheck} />
              <KpiCard label="Pending >45 Days" value={total.pending_reg_45d}
                sub={`of ${total.pending_reg} total pending`}
                color={total.pending_reg_45d > 20 ? "var(--accent-red)" : "var(--accent-amber)"}
                icon={AlertCircle} />
            </div>

            {/* CRM Achievement gauge + weekly */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 16, marginTop: 16 }}>
                ...
                <div style={{ position: "relative", width: 180, height: 180 }}>
                  <svg viewBox="0 0 180 180" style={{ transform: "rotate(-90deg)" }}>
                    ...
                    <circle cx={90} cy={90} r={70} fill="none"
                      stroke={pctColor(data.crm_pct)}
                      strokeWidth={14}
                      strokeLinecap="round"
                      strokeDasharray={`${2 * Math.PI * 70}`}
                      strokeDashoffset={`${2 * Math.PI * 70 * (1 - data.crm_pct / 100)}`}
              ...
              {/* Collection by project bar chart */}
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={data.projects} barSize={28}>
                    ...
                    <XAxis dataKey="name" tick={{ fontSize: 10, fill: "var(--text-muted)" }}
                      tickFormatter={v => v.replace("RAGHAV ", "")} />
                    <YAxis tick={{ fontSize: 10, fill: "var(--text-muted)" }} />
                    <Tooltip content={<CustomTooltip />} />
                    <Bar dataKey="monthly_collection" name="Collection" radius={[4, 4, 0, 0]}>
                      {data.projects.map((p, i) => (
                        <Cell key={i} fill={projectColors[p.name] || "var(--accent-blue)"} fillOpacity={0.85} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
            ...
            {/* Outstanding + Pending Regs by project */}
              ...
              <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 24 }}>
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={data.projects.filter(p => p.outstanding > 0)} layout="vertical" barSize={16}>
                    ...
                    <YAxis dataKey="name" type="category" width={90} tick={{ fontSize: 10, fill: "var(--text-muted)" }}
                      tickFormatter={v => v.replace("RAGHAV ", "")} />
                    <Tooltip content={<CustomTooltip />} />
                    ...
                    <div key={p.name}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 12 }}>
                        ...
                        <span style={{ fontWeight: 600 }}>
                          {p.pending_reg} <span style={{ color: "var(--accent-red)", fontSize: 11 }}>({p.pending_reg_45d} &gt;45d)</span>
                        </span>
                      </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                ...
                <tbody>
                  {[...data.projects, { ...data.total, name: "TOTAL", _isTotal: true }].map((p, i) => (
                    <tr key={p.name}
                      style={{ background: p._isTotal ? "rgba(79,179,255,0.05)" : i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.015)", transition: "background 0.15s" }}
        ...
        {activeTab === "targets" && (
          <>
            <SectionTitle icon={Layers}>Project-wise Target vs Achievement</SectionTitle>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 24 }}>
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={data.project_tva.filter(p => p.target > 0 || p.achievement > 0)} barCategoryGap="25%">
                  ...
                  <Bar dataKey="target" name="Target" fill="rgba(79,179,255,0.25)" stroke="var(--accent-blue)" strokeWidth={1} radius={[4, 4, 0, 0]} />
                  <Bar dataKey="achievement" name="Achievement" fill="var(--accent-green)" fillOpacity={0.85} radius={[4, 4, 0, 0]} />
                  <Bar dataKey="forecast" name="Forecast" fill="var(--accent-amber)" fillOpacity={0.7} radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12, marginTop: 20 }}>
              {data.project_tva.filter(p => p.target > 0).map(p => (
                <div key={p.name} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: 16 }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 6 }}>{cr(p.achievement)} / {cr(p.target)}</div>
                  <div style={{ fontSize: 11, color: "var(--accent-amber)", marginTop: 2 }}>Forecast: {cr(p.forecast)}</div>
                </div>
              ))}
            </div>

            <SectionTitle icon={Layers}>Category-wise Target vs Achievement</SectionTitle>
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
              <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 24 }}>
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={data.category_tva} layout="vertical">
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" horizontal={false} />
                    <XAxis type="number" tick={{ fontSize: 10, fill: "var(--text-muted)" }} />
                    <YAxis dataKey="category" type="category" width={150} tick={{ fontSize: 10, fill: "var(--text-muted)" }} />
                    <Tooltip content={<CustomTooltip />} />
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {data.category_tva.map(c => (
                  <div key={c.category} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "12px 16px" }}>
                    <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 4 }}>{c.category}</div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                      <span>T: <strong>{cr(c.target)}</strong></span>
        ...
        {activeTab === "trends" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {/* CRM Target vs Achievement trend */}
              <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 24 }}>
                <div style={{ fontSize: 12, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>CRM Target vs Achievement (₹ Cr)</div>
              ...
              {/* Live bookings & pending reg trend */}
              <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 24 }}>
                  ...
                  {trendData.map((d, i) => (
                    <div key={d.month}>
                      ...
                      <div style={{ display: "flex", gap: 16, marginTop: 4, fontSize: 11, color: "var(--text-muted)" }}>
                        <span>Target: ₹{d.target} Cr</span>
                        <span>Achieved: ₹{d.achievement} Cr</span>
                        <span>Collect: ₹{d.monthly_collection} Cr</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <button
        ...
        style={{
          ...
          background: chatOpen ? "var(--bg-card)" : "linear-gradient(135deg, var(--accent-blue), var(--accent-cyan))",
          border: "1px solid var(--border-accent)",
          boxShadow: "0 8px 32px rgba(79,179,255,0.3)",
